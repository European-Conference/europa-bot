...
let extract_prop = (text, prop) => {
  ...
  if (matching_props.length === 0) {
    throw new Error(
      `Could not extract the required property ${prop} from your message. I was looking for a line like '${prop}: <some_${prop}>', but could not find such a line in your messsage.`
    );
  }

  if (matching_props.length > 1) {
    throw new Error(
      `Found ${matching_props.length} ${prop} properties in your message, but I was expecting to find exactly one. There must be multiple lines that look like '${prop}: <some_${prop}>' in your message.`
    );
  }
...
async function process_image(url) {
  ...
  const fileName = `${Math.random()
    .toString(36)
    .substring(2, 15)}${path.extname(url)}`;
  ...
  if (image.bitmap.width < 300) {
    throw new Error(
      `Your image is too small (only ${image.bitmap.width} wide). Only images that are at least 300 pixels wide are accepted, or they won't look good on large screens.`
    );
  }

  if (aspectRatio < 0.75 || aspectRatio > 1) {
    throw new Error(
      `Your image has an incompatible aspect ratio. The aspect ratio was ${aspectRatio}, but only aspect ratios between 0.75 and 1 are accepted, so that the images look good on the website.`
    );
  }
  ...
  if (config.RESIZE_IMAGES) image.resize(300, Jimp.AUTO).write(filePath);
...
let extract_speaker = async (event) => {
  let text = event.text;
  let type = extract_prop(text, "type");
  if (type !== "speaker" && type !== "organizer") {
    throw new Error(
      `The type property must be either 'speaker' or 'organizer', but it was '${type}'.`
    );
  }
...
async function create_speaker_pr(speaker) {
  ...
  await gitRepo.checkoutLocalBranch(`${speaker.type}s/` + speaker.id);
  try {
    await gitRepo.pull("origin", `${speaker.type}s/` + speaker.id);
  } catch (e) {
    console.log("Ignoring git pull errror: " + e.message);
  }

  console.log("Making changes...");
  // Create the content/[organizer|speaker]s/{speaker.id} directory if it doesn't exist
...
app.post("/events", async (req, res) => {
  ...
  if (event.text.includes("```")) {
    slack_error(
      "I can't parse code blocks: your message contained\n```code blocks```\n instead of plain text.",
      event.ts
    );
    return;
  }
  ...
  slack_thread_message(
    `Hello human! Okay, I'll create a Pull Request to change (or update) person ${speaker.id}. Hold on, this might take a minute.`,
    event.ts
  );

  try {
    console.log(`Creating PR for person ${speaker.id}...`);
    ...
    if (/A pull request already exists for (.*)\./.test(e.message)) {
      ...
      await slack_thread_message(
        `It seems that a Pull Request (${pr}) already exists for this person. I've updated it, so you're good to go!`,
        event.ts
      );
      return;
    }
    ...
    await slack_thread_message(
      `Unfortunately, something went wrong while trying to create the PR. It's probably not your fault, human.`,
      event.ts
    );

    console.log("Error while creating person PR:");
    console.log(e);
    return;
  }

  await slack_thread_message(
    `Your PR has been opened (great success!): ${pr}. Now find some wise human to review it (@Christos).`,
    event.ts
  );
});
